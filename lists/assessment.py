...
def all_odd(numbers):
    ...
    # list comprehension allows for list iteration in one line of code
...
def print_indices(items):
    ...
    # enumerate function assigns index to each item in items
...
def foods_in_common(foods1, foods2):
    ...
    **NOTE**: for this problem, you're welcome to use any of the
    ...
    # converted both lists of foods into sets
    # this allows for set math to check for foods in common
    set_foods1 = set(foods1)

    set_foods2 = set(foods2)

    # sorted puts items into alphabetical order
    sorted_foods_in_common = list(sorted((set_foods1 & set_foods2)))

    return sorted_foods_in_common
...
def every_other_item(items):
    ...
    # indexing allows us to step by 2 to find every other word
    # start and end index not necessary if we want to step entire list
    return items[::2]
...
def largest_n_items(items, n):
    ...
    # will return empty list if n is 0
    if n == 0:
        return []

    else:
        sorted_items = sorted(items)
        # after sorting largest n items will be the last n items in the list
        n_largest_items = sorted_items[-n:]
